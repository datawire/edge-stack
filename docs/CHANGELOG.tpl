...
## [{{ .version }}] {{ .date }}
[{{ .version }}]: https://github.com/datawire/edge-stack/releases/v{{ .version }}
{{- range .notes }}{{ if index . "isHeadline" }}{{ if .isHeadline }}

{{ .body | strings.ReplaceAll "$productName$" "Ambassador Edge Stack" | strings.ReplaceAll "<b>" "_" | strings.ReplaceAll "</b>" "_" | strings.ReplaceAll "<code>" "`" | strings.ReplaceAll "</code>" "`" | strings.WordWrap 98 }}
{{- end }}{{ end }}{{ end }}

## Ambassador Edge Stack
{{ range .notes -}}{{ if not (index . "isHeadline") }}
- {{ .type | strings.Title }}: {{ .body | strings.ReplaceAll "$productName$" "Ambassador Edge Stack" | strings.ReplaceAll "<b>" "_" | strings.ReplaceAll "</b>" "_" | strings.ReplaceAll "<code>" "`" | strings.ReplaceAll "</code>" "`" | strings.WordWrap 98 "\n  " }}{{ if index . "github" }}{{ range .github }} ([{{.title}}]){{ end }}{{ end }}
